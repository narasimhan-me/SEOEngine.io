 ...
 * - Event ID tracking for full idempotency
 */
...
describe('BILLING-1 – Billing Webhook Idempotency (Critical)', () => {
  ...
  describe('checkout.session.completed idempotency with event ID tracking', () => {
    it('should create subscription and track event ID on first checkout.session.completed event', async () => {
      ...
      // Verify subscription was created with event ID tracking
      ...
      expect(subscription?.lastStripeEventId).toBe(eventId); // Event ID should be tracked
    });

    it('should be idempotent when same checkout.session.completed event is processed twice (event ID tracking)', async () => {
      const { user } = await createTestUser(testPrisma, { plan: 'free' });
      ...
      expect(subscriptionAfterFirst?.lastStripeEventId).toBe(eventId);
      ...
      // Verify event ID tracking prevents duplicate processing
      expect(subscriptionAfterSecond?.lastStripeEventId).toBe(eventId);
    });

    it('should prevent double-charging by using event ID tracking for checkout.session.completed', async () => {
      const { user } = await createTestUser(testPrisma, { plan: 'free' });
      ...
      expect(subscriptions[0]?.lastStripeEventId).toBe(eventId);
    });
  });
