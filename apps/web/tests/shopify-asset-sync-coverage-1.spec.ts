...
async function connectShopifyE2EWithScope(
  request: any,
  projectId: string,
  scope: string
) {
...
async function setShopifyScopeE2E(
  request: any,
  projectId: string,
  scope: string
) {
  const res = await request.post(
    `${API_BASE_URL}/testkit/e2e/set-shopify-scope`,
    {
      data: { projectId, scope },
    }
  );
...
async function seedMockShopifyAssets(request: any) {
  const res = await request.post(
    `${API_BASE_URL}/testkit/e2e/mock-shopify-assets`,
    {
      data: {
        pages: [
          {
...
test.describe('SHOPIFY-ASSET-SYNC-COVERAGE-1: Shopify Pages + Collections Sync', () => {
  test('SHOPIFY-SCOPE-RECONSENT-UX-1-FIXUP-1: Reconnect CTA never fails silently when token missing', async ({
    ...
    await connectShopifyE2EWithScope(
      request,
      projectId,
    ...
    await expect(
      page.getByRole('heading', {
        name: 'Additional Shopify permission required',
      })
    ).toBeVisible();
    ...
    await expect(
      page.getByText(
        /session token is missing\. Please sign in again, then retry\./i
      )
    ).toBeVisible();
    await expect(
    ...
    await expect(page).toHaveURL(
      new RegExp(`/projects/${projectId}/assets/pages`)
    );
  });
  ...
  test('SHOPIFY-SCOPE-RECONSENT-UX-1-FIXUP-1: Reconnect CTA calls reconnect-url and navigates to Shopify OAuth', async ({
    ...
    expect(reconnectReqUrl.searchParams.get('returnTo')).toBe(
  ...
  test('SHOPIFY-SCOPE-RECONSENT-UX-1: Missing scope shows permission notice and auto-syncs after reconnect return', async ({
    ...
    const syncCollections = page.getByRole('button', {
      name: /Sync Collections/i,
    });
    ...
    await setShopifyScopeE2E(
      request,
      projectId,
      'read_products,write_products,read_content'
    );
    await page.goto(
      `/projects/${projectId}/assets/pages?shopify=reconnected&reconnect=pages_sync`
    );
    await expect(page.getByText(/Last synced:/i)).toBeVisible({
      timeout: 15000,
    });
