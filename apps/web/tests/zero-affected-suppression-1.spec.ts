...
test.describe('ZERO-AFFECTED-SUPPRESSION-1 – Zero-eligible trust hardening', () => {
  test('Work Queue suppresses 0-eligible automation bundles (no actionable tiles, no CTAs)', async ({
    ...
    const { projectId, accessToken } =
      await seedWorkQueueZeroEligibleDraft(request);
    ...
    await expect(page.getByText(/Fix missing product SEO titles/i)).toHaveCount(
      0
    );
    await expect(
      page.getByRole('link', {
  ...
  test('Playbooks shows calm empty state at 0 eligible (no stepper, no apply semantics)', async ({
    ...
    await authenticatePage(page, accessToken);
    await page.goto(
      `/projects/${projectId}/automation/playbooks?playbookId=missing_seo_title`
    );
    await page.waitForLoadState('networkidle');
    await expect(
      page.getByTestId('playbook-zero-eligible-empty-state')
    ).toBeVisible();
