...
async function seedInsightsProject(request: any) {
  const res = await request.post(`${API_BASE_URL}/testkit/e2e/seed-insights-1`, {
    data: {},
  });
...
test.describe('INSIGHTS-1 – Insights Dashboard Smoke Test', () => {
  test('Insights page renders with heading, subnav, and overview cards', async ({
    ...
    await expect(
      page.locator('[data-testid="insights-subnav"]'),
    ).toBeVisible();

    // Assert: At least one overview card label visible (Improved, Saved, Resolved, or Next)
    const hasImprovedCard = await page.getByText(/Improved/i).isVisible().catch(() => false);
    const hasSavedCard = await page.getByText(/Saved/i).isVisible().catch(() => false);
    const hasResolvedCard = await page.getByText(/Resolved/i).isVisible().catch(() => false);
    const hasNextCard = await page.getByText(/Next/i).isVisible().catch(() => false);

    expect(hasImprovedCard || hasSavedCard || hasResolvedCard || hasNextCard).toBeTruthy();
  });
});
