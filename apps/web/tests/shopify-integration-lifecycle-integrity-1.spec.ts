...
async function setProjectDomain(
  request: any,
  accessToken: string,
  projectId: string,
  domain: string
...
async function login(page: any, token: string) {
  ...
  await page.evaluate(
    (t: string) => localStorage.setItem('engineo_token', t),
    token
  );
}
...
test.describe('SHOPIFY-INTEGRATION-LIFECYCLE-INTEGRITY-1', () => {
  test('SILI1-001: Disconnect Shopify clears connected state across Settings and Store Health', async ({
    ...
    await expect(
      page.getByRole('heading', { name: 'Active Integrations' })
    ).toBeVisible();
    await expect(
      page.getByRole('button', { name: 'Disconnect Shopify' })
    ).toBeVisible();
    await page.getByRole('button', { name: 'Disconnect Shopify' }).click();
    await expect(
      page.getByRole('button', { name: 'Connect Shopify' })
    ).toBeVisible();
  ...
  test('SILI1-003: Products empty-state link routes to /settings#integrations', async ({
    ...
    const projectId = await createProject(
      request,
      accessToken,
      `empty-${Date.now()}.myshopify.com`
    );
    await login(page, accessToken);

    await page.goto(`/projects/${projectId}/products`);
    const link = page.getByRole('link', {
      name: /Go to project settings to connect Shopify/i,
    });
    await expect(link).toBeVisible();
    await link.click();
    await expect(page).toHaveURL(
      new RegExp(`/projects/${projectId}/settings#integrations`)
    );
  });
  ...
  test('SILI1-004: Settings Connect Shopify uses connect-url and never fails silently when token missing', async ({
    ...
    const projectId = await createProject(
      request,
      accessToken,
      `connect-${Date.now()}.myshopify.com`
    );
    await login(page, accessToken);
    ...
    await expect(
      page.getByRole('button', { name: /Sign in again/i })
    ).toBeVisible();
    ...
    const connectRequestPromise = page.waitForRequest((req) => {
      return (
        req.method() === 'GET' &&
        req.url().includes(`/projects/${projectId}/shopify/connect-url`)
      );
    });
    ...
    expect(connectReqUrl.searchParams.get('returnTo')).toBe(
      `/projects/${projectId}/settings#integrations`
    );

    await oauthNavPromise;
    const oauthUrl = new URL(page.url());
    expect(oauthUrl.searchParams.get('redirect_uri') || '').toContain(
      '/shopify/callback'
    );
    expect((oauthUrl.searchParams.get('scope') || '').length).toBeGreaterThan(
      0
    );
  });
});
