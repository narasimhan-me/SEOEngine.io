...
function calculateNextCrawl(
  ...
  const nextCrawl = new Date(
    lastCrawl.getTime() + daysToAdd * 24 * 60 * 60 * 1000
  );
...
export default function ProjectSettingsPage() {
  ...
  const [reconnectCapability, setReconnectCapability] = useState<
    'pages_sync' | 'collections_sync' | null
  >(null);
  ...
  const [autoSuggestMissingMetadata, setAutoSuggestMissingMetadata] =
    useState(false);
  ...
  const [
    ...
    setAutoGenerateAnswerBlocksOnProductSync,
  ] = useState(false);
  ...
  const fetchIntegrationStatus = useCallback(async () => {
    try {
      ...
      setAutoGenerateAnswerBlocksOnProductSync(
        data.autoGenerateAnswerBlocksOnProductSync ?? false
      );
      if (data?.shopify?.connected) {
        ...
        const collectionsMissing =
          (collectionsScope as any)?.missingScopes ?? [];
        const combined = Array.from(
          new Set([...(pagesMissing ?? []), ...(collectionsMissing ?? [])])
        );
        setShopifyMissingScopes(combined);
        setReconnectCapability(
          (pagesMissing?.length ?? 0) > 0
            ? 'pages_sync'
            : (collectionsMissing?.length ?? 0) > 0
              ? 'collections_sync'
              : null
        );
      } else {
  ...
  const handleDisconnectShopify = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "We couldn't disconnect Shopify. Please try again.";
  ...
  const handleReconnectShopify = useCallback(async () => {
    ...
    if (!reconnectCapability) {
      setReconnectError(
        "We couldn't determine which Shopify permission is missing. Please refresh and try again."
      );
      setReconnectingShopify(false);
      return;
    }
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "We couldn't start Shopify reconnection. Please try again.";
  ...
  const handleSetupMetafields = async () => {
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : 'Failed to set up metafield definitions';
      setError(message);
      feedback.showError(message);
    } finally {
    ...
    <div>
      ...
      <div className="rounded-lg bg-white p-6 shadow mb-6">
        ...
        <p className="text-sm text-gray-600 mb-6">
          Configure how often EngineO automatically crawls your site to update
          DEO scores and detect issues.
        </p>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <label
                htmlFor="autoCrawlEnabled"
                className="text-sm font-medium text-gray-900"
              >
                Enable Automatic Crawling
              </label>
              <p className="text-sm text-gray-500 mt-0.5">
                When enabled, EngineO will crawl your site on the configured
                schedule.
              </p>
            </div>
            <button
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setCrawlFrequency(e.target.value as CrawlFrequency)
              }
      ...
      <div className="rounded-lg bg-white p-6 shadow mb-6">
        ...
        <p className="text-sm text-gray-600 mb-6">
          Configure automatic AI suggestion generation after each crawl.
          Suggestions are created as drafts and never auto-applied.
        </p>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                Generate AI suggestions for pages and products missing SEO
            ...
            <button
              ...
              onClick={() =>
                setAutoSuggestMissingMetadata(!autoSuggestMissingMetadata)
              }
              className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                Generate AI suggestions for pages with less than 200 words or
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                When enabled, Answer Blocks can be synced to Shopify as
                metafields for each product.
              </p>
            </div>
            <button
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                When enabled, syncing products from Shopify may enqueue AI
                generation for Answer Blocks.
              </p>
            </div>
            <button
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm text-blue-700">
                  AI suggestions are generated as drafts after each crawl. You
                  can review and apply them from the Products or Content pages.
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                <h3 className="text-sm font-medium text-red-800">
                  Could not verify Shopify permissions
                </h3>
                <p className="mt-1 text-sm text-red-700">
                  We were unable to verify the permissions granted by Shopify.
                  This is usually temporary. Please try connecting again.
                </p>
                <button
                  type="button"
                  onClick={() => {
          ...
          <div className="mb-4 rounded-md border border-gray-200 bg-gray-50 p-4">
            ...
            <div className="mt-3 flex items-center gap-3">
              <button
                ...
                disabled={
                  (capabilities ? !capabilities.canModifySettings : false) ||
                  connectingShopify
                }
              ...
              {capabilities && !capabilities.canModifySettings && (
                <span className="text-xs text-gray-500">
                  Ask a project owner to connect Shopify.
                </span>
              )}
            </div>
            {connectError && (
        ...
        {status.integrations.length > 0 ? (
                ...
                {integration.type === 'SHOPIFY' && (
                  <div className="mt-3 pt-3 border-t border-gray-200">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Creates metafield definitions in Shopify for Answer Block
                      sync.
                    </p>
                    ...
                    <div className="mt-3 flex items-center gap-3">
                      ...
                      {capabilities && !capabilities.canModifySettings && (
                        <span className="text-xs text-gray-500">
                          Ask a project owner to disconnect Shopify.
                        </span>
                      )}
                    </div>
                    {disconnectError && (
          ...
          <div className="text-center py-6">
            ...
            <p className="text-xs text-gray-400 mt-1">
              Connect Shopify to enable sync features.
            </p>
          </div>
        )}
      </div>
