    ...
    <div>
      ...
      {data?.queueHealth.jobLagWarning && (
        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-6 py-4 rounded-lg mb-6">
          ...
          <p className="mt-1">
            {data.queueHealth.stalledRuns} run(s) have been queued for more than
            1 hour.
          </p>
        </div>
      )}
            ...
            <div className="flex justify-between items-center">
              ...
              <span
                className={`text-2xl font-bold ${
                  (data?.queueHealth.stalledRuns || 0) > 0
                    ? 'text-yellow-600'
                    : 'text-green-600'
                }`}
              >
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span
                className={`text-2xl font-bold ${
                  (data?.failureSignals.recentFailures || 0) > 0
                    ? 'text-red-600'
                    : 'text-green-600'
                }`}
              >
            ...
            <div className="flex justify-between items-center">
              ...
              <span
                className={`text-2xl font-bold ${
                  (data?.failureSignals.shopifyFailures24h || 0) > 0
                    ? 'text-red-600'
                    : 'text-green-600'
                }`}
              >
      ...
      <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded text-sm">
        Last checked:{' '}
        {data?.checkedAt ? new Date(data.checkedAt).toLocaleString() : 'N/A'}
      </div>
    </div>
  );
}
