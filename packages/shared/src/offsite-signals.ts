 ...
 *
 * This module defines the off-site signal taxonomy, coverage models,
 * gap types, and fix flow types for the Off-site Signals pillar.
 *
 * ETHICAL BOUNDARIES (Critical):
 * - Off-site signals are modeled as presence/absence and examples, not as authority metrics.
 * - No raw scraped content or DA-like scores are included in v1.
 * - No scraping of third-party websites or competitor content.
 * - The model describes presence and quality of trust signals, not raw backlink counts.
 * - All generated content is for outreach/PR drafts that require human review.
 */
...
/**
 * Off-site fix preview request payload.
 */
export interface OffsiteFixPreviewRequest {
  /** Gap type to address */
  gapType: OffsiteGapType;
  /** Signal type */
  signalType: OffsiteSignalType;
  /** Focus key (e.g., "reviews/trustpilot", "directory:industry_comparison") */
  focusKey: string;
  /** Desired draft type */
  draftType: OffsiteFixDraftType;
}
...
/**
 * Off-site fix preview response.
 */
export interface OffsiteFixPreviewResponse {
...
/**
 * Off-site fix apply request payload.
 */
export interface OffsiteFixApplyRequest {
  /** Draft ID to apply */
  draftId: string;
...
/**
 * Off-site fix apply response.
 */
export interface OffsiteFixApplyResponse {
  /** Whether the apply was successful */
  success: boolean;
  /** Updated coverage summary */
  updatedCoverage: ProjectOffsiteCoverage;
  /** Whether related issues are resolved or marked as "action taken" */
