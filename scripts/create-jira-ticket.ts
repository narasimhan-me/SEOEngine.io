      ...
      issueTypes.find(
        (it) =>
          ...
          (it.name.toLowerCase().includes('tech') &&
            data.issueType.toLowerCase().includes('tech'))
      ) || issueTypes[0];
...
async function main() {
  ...
  if (!ticketKey) {
    console.error(
      'Usage: ts-node scripts/create-jira-ticket.ts <TICKET-KEY> [--project <PROJECT-KEY>]'
    );
    console.error(
      'Example: ts-node scripts/create-jira-ticket.ts ISSUE-ACTION-DESTINATION-GAPS-1 --project KAN'
    );
    console.error(
      'Example: ts-node scripts/create-jira-ticket.ts PRODUCT-IDEA-1 --project EA'
    );
    console.error('\nProject Key Guidelines:');
  ...
  try {
    ...
    const finalProjectKey =
      projectKey || process.env.JIRA_PROJECT_KEY || 'ENGINEO';

    console.log(`🔗 Connecting to Jira: ${process.env.JIRA_BASE_URL}`);
    console.log(
      `📦 Project Key: ${finalProjectKey}${projectKey ? ' (from --project flag)' : process.env.JIRA_PROJECT_KEY ? ' (from .env)' : ' (default)'}`
    );
    ...
    if (!connectionOk) {
      console.error(
        `❌ Failed to connect to Jira. Please check your credentials and Jira instance URL.`
      );
      process.exit(1);
    }
...
if (process.argv[2] === 'update') {
  ...
  if (!issueKey) {
    ...
    console.error(
      'Example: ts-node scripts/create-jira-ticket.ts update EA-14 --project EA'
    );
    process.exit(1);
  }
  ...
  (async () => {
    try {
      ...
      const scriptPath =
        process.argv[1] || require.main?.filename || __filename;
      const scriptDir = dirname(scriptPath);
      const projectRoot = join(scriptDir, '..');
