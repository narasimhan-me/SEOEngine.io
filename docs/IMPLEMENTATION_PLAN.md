...
| 7.55 | 2026-01-25 | **ERROR-&-BLOCKED-STATE-UX-1 FIXUP-1**: Inline blocked explanation (visible without hover). (1) Updated Issues page blocked chip render: wrapped chip in flex container with visible description text below; description sourced from `blockedCopy.description`; styling is token-only (text-[10px] leading-snug text-muted-foreground); tooltip retained for full context + nextStep; (2) Accessibility check: description is plain text (no ARIA needed), DOM order is chip → description (correct screen-reader order); (3) Dev-time guardrail updated: comment clarifies warning fires when inline explanation would be empty; (4) Manual testing doc updated with Scenario 7 (EBSUX1-HP-007) for inline blocked explanation verification. **No backend changes.** **Core files:** issues/page.tsx, ERROR-&-BLOCKED-STATE-UX-1.md. **Critical Path:** CP-020 updated (6.68). |
