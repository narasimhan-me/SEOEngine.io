...
- [ ] ERROR-&-BLOCKED-STATE-UX-1: Issues Engine blocked chips use centralized deriveBlockedState() (not ad-hoc reasonBlocked)
- [ ] ERROR-&-BLOCKED-STATE-UX-1: Blocked chip label comes from getBlockedStateCopy().chipLabel (e.g., "Blocked — permissions")
- [ ] ERROR-&-BLOCKED-STATE-UX-1: Blocked chip tooltip shows description + nextStep as single readable sentence
- [ ] ERROR-&-BLOCKED-STATE-UX-1: Blocked chip has data-no-row-click attribute (clicking chip does not open RCP)
- [ ] ERROR-&-BLOCKED-STATE-UX-1: No Fix/Apply CTA visible when deriveBlockedState() returns non-null
- [ ] ERROR-&-BLOCKED-STATE-UX-1: RCP "Why this is blocked" line appears in Actionability section when blocked
- [ ] ERROR-&-BLOCKED-STATE-UX-1: RCP "Why this is blocked" content matches table tooltip (description + nextStep)
- [ ] ERROR-&-BLOCKED-STATE-UX-1: Sync-pending empty state shows warning-styled message (not success/healthy)
- [ ] ERROR-&-BLOCKED-STATE-UX-1: RCP error state uses SYSTEM_ERROR-aligned copy ("This action is unavailable due to a system error...")
- [ ] ERROR-&-BLOCKED-STATE-UX-1: Token-only styling preserved (no in-body navigation links)
- [ ] ERROR-&-BLOCKED-STATE-UX-1 FIXUP-1: Blocked state suppresses Fix CTA even when a fix destination exists (e.g., Shopify scope missing)
- [ ] ERROR-&-BLOCKED-STATE-UX-1 FIXUP-2: Uses integrationStatusOk (not integrationStatusLoaded) for blocked state gating; only infers SYNC_PENDING when lastCrawledAt === null and integrationStatusOk === true
- [ ] ERROR-&-BLOCKED-STATE-UX-1 FIXUP-3: RCP mismatch warning only fires when integrationStatusOk === true (no spurious warnings when integration data not trusted)
- [ ] ERROR-&-BLOCKED-STATE-UX-1 FIXUP-1: Inline blocked explanation visible below chip without hover (description text from blockedCopy.description)
...
| 6.61 | 2026-01-25 | DRAFT-LIFECYCLE-VISIBILITY-1 FIXUP-1: RCP Echo + Conservative Derivation. **PATCH 1:** NO_DRAFT now has displayable shortLabel ("No draft exists") so RCP can render meaningful draft line. **PATCH 2:** Conservative APPLIED derivation: `deriveDraftLifecycleState()` now requires explicit `hasAppliedSignal === true` to return APPLIED; legacy `legacyDraftState === 'applied'` alone no longer elevates to APPLIED (trust guard). **PATCH 3:** Issues descriptor passes `metadata.draftLifecycleState` to RCP via `getIssueDescriptor()` for accurate echo during preview states. **PATCH 4:** RCP always shows draft lifecycle line (including NO_DRAFT: "Draft: No draft exists") for complete state visibility (removed NO_DRAFT gating). **PATCH 5:** Manual testing doc Scenario 1 updated to expect "Draft: No draft exists" in RCP. **PATCH 6:** Updated IMPLEMENTATION_PLAN.md with FIXUP-1 entry. **Core files:** draftLifecycleState.ts, page.tsx, ContextPanelIssueDetails.tsx, DRAFT-LIFECYCLE-VISIBILITY-1.md. **Phase DRAFT-LIFECYCLE-VISIBILITY-1 FIXUP-1 complete.** |
| 6.62 | 2026-01-25 | ISSUE-FIX-KIND-CLARITY-1 FIXUP-4: Preview Eligibility + View Affected Label. **PATCH 1:** Created `lib/issues/inlineAiPreviewSupport.ts` with centralized allowlist (`missing_seo_title`, `missing_seo_description`) and `isInlineAiPreviewSupportedIssueType()` helper (single source of truth for inline preview eligibility). **PATCH 2:** Tightened AI_PREVIEW_FIX derivation in `issueFixActionKind.ts`: now requires `isInlineAiPreviewSupportedIssueType(issueType)` check to prevent "Review AI fix" CTA appearing for issues without inline preview UI. **PATCH 3:** Fixed viewAffected mislabel in `getIssueFixActionKindInfo()`: GUIDANCE_ONLY issues with viewAffected (no fix) now show "View affected" (exploration label) instead of "Review guidance". **PATCH 4:** Updated Issues page to import and use centralized helper (replaced inline `supportsInlineFix` check). **PATCH 5:** Added dev-time guardrail: console warning if AI_PREVIEW_FIX derived but `supportsInlineFix` is false (catches allowlist drift). **PATCH 6:** Updated ISSUE-FIX-KIND-CLARITY-1.md with FIXUP-4 scenarios (F4-001, F4-002) and verification checklist. **PATCH 7:** Playwright regression check passed (existing assertions compatible). **PATCH 8:** Updated IMPLEMENTATION_PLAN.md with FIXUP-4 entry. **Core files:** inlineAiPreviewSupport.ts (new), issueFixActionKind.ts, page.tsx, ISSUE-FIX-KIND-CLARITY-1.md. **Phase ISSUE-FIX-KIND-CLARITY-1 FIXUP-4 complete.** |
| 6.63 | 2026-01-25 | ISSUE-FIX-KIND-CLARITY-1 FIXUP-4 (completion): viewAffected CTA uses fixActionKindInfo. **PATCH 1:** Updated Issues page viewAffected CTA block to use `fixActionKindInfo.label`, `fixActionKindInfo.iconKey`, and `fixActionKindInfo.sublabel` instead of hardcoded values. Prevents UI/derivation drift. Changed comment from "Review guidance" to "View affected - exploration (not guidance)". **Core files:** page.tsx. **Phase ISSUE-FIX-KIND-CLARITY-1 FIXUP-4 fully complete.** |
| 6.64 | 2026-01-25 | ERROR-&-BLOCKED-STATE-UX-1: Centralized blocked-state derivation and trust-safe error copy. **PATCH 1:** Created `lib/issues/blockedState.ts` with 6 canonical blocked states (PERMISSIONS_MISSING, SHOPIFY_SCOPE_MISSING, DRAFT_REQUIRED, DESTINATION_UNAVAILABLE, SYNC_PENDING, SYSTEM_ERROR); `deriveBlockedState()` uses existing UI signals (no new backend calls); `getBlockedStateCopy()` returns chipLabel/description/nextStep; private Shopify scope parser handles write_* satisfying read_*. **PATCH 2:** Updated Issues page blocked chip: uses `deriveBlockedState()`, displays `copy.chipLabel` (e.g., "Blocked — permissions"), tooltip shows description + nextStep, `data-no-row-click` attribute prevents RCP open on chip click, no Fix/Apply CTA when blocked. **PATCH 3:** Updated ContextPanelContentRenderer to pass blockedState/shopifyConnected/shopifyScope/lastCrawledAt metadata to ContextPanelIssueDetails; RCP derives blocked state locally and shows "Why this is blocked" line in Actionability section when blocked. **PATCH 4:** Updated Issues page empty state: sync-pending (no lastCrawledAt) shows warning-styled message with SYNC_PENDING copy instead of success/healthy; RCP error state uses SYSTEM_ERROR-aligned copy. **PATCH 5:** Created ERROR-&-BLOCKED-STATE-UX-1.md manual testing doc (6 scenarios, blocked chip behavior checks, edge cases). **PATCH 6:** Added dev-time guardrails (console.warn if Fix/Apply CTA renders while blocked, if blocked state has no description copy, if RCP vs table blocked state mismatch). **Core files:** blockedState.ts (new), page.tsx, ContextPanelContentRenderer.tsx, ContextPanelIssueDetails.tsx. **No backend changes.** Token-only styling. Added 11 checklist items to CP-020. **Phase ERROR-&-BLOCKED-STATE-UX-1 complete.** |
| 6.65 | 2026-01-25 | ERROR-&-BLOCKED-STATE-UX-1 FIXUP-1: Enforce blocked gating + integration-status-load semantics. **PATCH 1:** Blocked state computed FIRST in Actions cell, before any Fix/View/Open CTA checks; if blocked, render blocked chip and return early (no Fix CTA ever shown for blocked issues). **PATCH 2:** Added `integrationStatusLoaded` state to prevent false SYNC_PENDING/SHOPIFY_SCOPE_MISSING while integration-status is loading; `deriveBlockedState()` only infers SYNC_PENDING when lastCrawledAt is explicitly `null` (known missing), not `undefined` (unknown). **PATCH 3:** Sync-pending empty state only shown when `integrationStatusLoaded === true`; `integrationStatusLoaded` passed through RCP metadata chain for aligned blocked state derivation. **PATCH 4:** Fixed IMPLEMENTATION_PLAN.md entry 7.51 (6.54→6.64); added 7.52 entry for FIXUP-1. **PATCH 5:** Manual testing doc updated with scenario for blocked state replacing Fix CTA. **Core files:** blockedState.ts, page.tsx, ContextPanelContentRenderer.tsx, ContextPanelIssueDetails.tsx, ERROR-&-BLOCKED-STATE-UX-1.md. Added FIXUP-1 checklist item to CP-020. **Phase ERROR-&-BLOCKED-STATE-UX-1 FIXUP-1 complete.** |
| 6.66 | 2026-01-25 | ERROR-&-BLOCKED-STATE-UX-1 FIXUP-2: Distinguish "loaded" from "trusted" integration status. **PATCH 1:** Replaced `integrationStatusLoaded` with `integrationStatusOk` (true only on successful fetch with trusted data); `lastCrawledAt` initialization changed from `null` to `undefined` (unknown vs known-missing semantics). **PATCH 2:** Blocked state derivation gating uses `integrationStatusOk` instead of `integrationStatusLoaded`; sync-pending empty state gating uses `integrationStatusOk && lastCrawledAt === null`. **PATCH 3:** Updated RCP prop chain: `integrationStatusOk` passed through ContextPanelContentRenderer to ContextPanelIssueDetails; renamed prop from `integrationStatusLoaded` to `integrationStatusOk`. **PATCH 4:** Removed unused `integrationStatusLoaded` state and `setIntegrationStatusLoaded` calls; removed `finally` block from fetch (integrationStatusOk only set on success). **PATCH 5:** Documentation updates: IMPLEMENTATION_PLAN.md 7.53, CRITICAL_PATH_MAP.md 6.66, ERROR-&-BLOCKED-STATE-UX-1.md updated with FIXUP-2 scenario. **Core files:** page.tsx, ContextPanelContentRenderer.tsx, ContextPanelIssueDetails.tsx. Added FIXUP-2 checklist item to CP-020. **Phase ERROR-&-BLOCKED-STATE-UX-1 FIXUP-2 complete.** |
| 6.67 | 2026-01-25 | ERROR-&-BLOCKED-STATE-UX-1 FIXUP-3: Suppress mismatch warning when integration status untrusted. **PATCH 1:** Updated `getIssueDescriptor()` in page.tsx: only compute `blockedState` when `integrationStatusOk === true`; pass empty strings for integration-dependent metadata fields when untrusted (prevents RCP from seeing stale/false values). **PATCH 2:** Updated ContextPanelIssueDetails.tsx: dev-time mismatch warning now gated on `isIntegrationOk === true` (prevents spurious warnings when neither table nor RCP have trusted data). **PATCH 3:** Documentation updates: IMPLEMENTATION_PLAN.md 7.54, CRITICAL_PATH_MAP.md 6.67, CP-020 checklist item added. **Core files:** page.tsx, ContextPanelIssueDetails.tsx. **Phase ERROR-&-BLOCKED-STATE-UX-1 FIXUP-3 complete.** |
| 6.68 | 2026-01-25 | ERROR-&-BLOCKED-STATE-UX-1 FIXUP-1: Inline blocked explanation (visible without hover). **PATCH 1:** Updated Issues page blocked chip render: wrapped chip in flex container, added visible description line below chip sourced from `blockedCopy.description`; styling is token-only (text-[10px] leading-snug text-muted-foreground); tooltip retained for full context. **PATCH 2:** Accessibility check: description is plain text (no ARIA attributes needed), DOM order is chip → description (correct screen-reader order). **PATCH 3:** Updated manual testing doc with Scenario 7 (EBSUX1-HP-007) for inline blocked explanation verification. **PATCH 4:** Dev-time guardrail comment updated to clarify warning fires when inline explanation would be empty. **PATCH 5:** Documentation updates: IMPLEMENTATION_PLAN.md 7.55, CRITICAL_PATH_MAP.md 6.68, CP-020 checklist item added. **Core files:** page.tsx, ERROR-&-BLOCKED-STATE-UX-1.md. **Phase ERROR-&-BLOCKED-STATE-UX-1 FIXUP-1 complete.** |
